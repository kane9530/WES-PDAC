...
file_dir <- "results_wes_full_40samples_pad50bp/vcfstats" 
...
mutect2_breakdown <- mutect2_df %>%
  ...
  scale_fill_manual(values = ArchR::paletteDiscrete(values = mymaf@variants.per.sample$Tumor_Sample_Barcode, 
                                                     set = "stallion", 
                                                     reverse = FALSE)) +
...
png("r_analysis_pad50bp/vcfstats/vcfstats_callers.png", res=300, units = "in", width = 20, height=10)
...
png("r_analysis_pad50bp/vcfstats/vcfstats_maxaf.png", res=300, units = "in", width = 10, height=10)
maxafimpact
dev.off()
```
...
## A) Union + remove dups + remove silent mutations

```{r}
mymaf <- read.maf("merged_mafs_310723_maftools.maf", removeDuplicatedVariants=TRUE)
mymaf@maf.silent
...
png("r_analysis_pad50bp/vcfstats/vcfstats_final_number_union.png", res=300, units = "in", width = 10, height=10)
final_number_union
dev.off()
```

## B) Intersection + remove dups + remove silent mutations

```{r}
mymaf <- read.maf("merged_mafs_intersect_210923_maftools.maf", removeDuplicatedVariants=TRUE)
mymaf@maf.silent
final_number_intersect <- mymaf@variants.per.sample %>%
  ggplot(aes(x= Tumor_Sample_Barcode, y=Variants, fill=Tumor_Sample_Barcode)) + 
  geom_bar(stat = "identity") +
 geom_text(aes(label = Variants), vjust=-0.5) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle=90), legend.position = "none") +
  ggtitle("Intersection of both callers and removal of silent variants") +
  scale_fill_manual(values = ArchR::paletteDiscrete(values = mymaf@variants.per.sample$Tumor_Sample_Barcode, set = "stallion", 
reverse = FALSE))

png("r_analysis_pad50bp/vcfstats/vcfstats_final_number_intersect.png", res=300, units = "in", width = 10, height=10)
final_number_intersect
dev.off()
```
...
bottom_row = cowplot::plot_grid(final_number_union, final_number_intersect, labels=c("C", "D"))

png("r_analysis_pad50bp/vcfstats/vcfstats_combined_all.png", res=300, units = "in", width = 20, height=18)
