...
merged_maf <- read.maf(paste0(res_path, "merged_mafs_allBatches_160124_maftools.maf"), 
                 verbose=TRUE)
merged_maf@data %>% dplyr::filter(Hugo_Symbol == "KRAS")  %>% dplyr::select(HGVSp_Short) %>% table()
merged_maf@data %>% dplyr::filter(Hugo_Symbol == "TP53")  %>% dplyr::select(HGVSp_Short) %>% table()
merged_maf@data %>% dplyr::filter(Hugo_Symbol == "TP53")  %>% dplyr::select(Tumor_Sample_Barcode, HGVSp_Short)
