...
```{r}
maf_files = c()
for (maf_file in list.files("/media/gedac/kane/projects/booney_wes/results_wes_full_40samples_pad50bp/combined/maf",
                            full.names=TRUE)){
...
# Intersction of both callers

```{r}
maf_files = c()
for (maf_file in list.files("/media/gedac/kane/projects/booney_wes/results_wes_full_40samples_pad50bp/combined/maf/intersect",
                            full.names=TRUE)){
  mymaf = read.maf(maf=maf_file)
  maf_files = append(maf_files, mymaf)
}
...
write.mafSummary(merged_maf, basename="merged_mafs_intersect_210923")
```

# Discard PCA117 PDO sample

```{r}
# Remove sample PCA117
tsb = mymaf@clinical.data$Tumor_Sample_Barcode[-grep(mymaf@clinical.data$Tumor_Sample_Barcode, pattern="PCA117_*")]
merged_maf_without_pca117 <- subsetMaf(merged_maf, tsb= tsb)
write.mafSummary(merged_maf_without_pca117, basename="merged_mafs_intersect_noPCA117_210923")
```
...
#merged_maf <- read.maf("merged_mafs_190923_maftools.maf", 
#                 verbose=TRUE)
merged_maf <- read.maf("merged_mafs_220923_noPCA117_maftools.maf", 
                 verbose=TRUE)
...
plotmafSummary(maf = merged_maf, rmOutlier = TRUE, addStat = 'median', dashboard = TRUE, titvRacdw = FALSE)
dev.off()
```
# Oncoplot 

```{r}
# Setting up annotation colors
...
lollipop_res <- "./r_analysis_pad50bp/lollipop/"
