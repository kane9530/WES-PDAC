...
res_path <-"r_analysis_pad50bp/tcga/"
```
...
(maftools_pdac@variants.per.sample)
...
# PDAC data
```{r}
merged_maf <- read.maf("merged_mafs_intersect_noPCA117_210923_maftools.maf", 
                 verbose=TRUE)
...
list_of_genes_all <- list(TCGA_PAAD = tcga_genes, PDO_TISSUE = pdac_genes)
...
png(paste0(res_path,"upset_allGenes.png"), units = "in", res = 300, width = 15, height = 12)
upset_all_genes
dev.off()
```
...
list_of_genes <- list(TCGA_PAAD = cosmic_genes_tcga, PDO_TISSUE = cosmic_genes)
...
png(paste0(res_path,"upset_cosmic_genes.png"), units = "in", res = 300, width = 15, height = 12)
upset_cosmic_genes
dev.off()
```
...
png(paste0(res_path,"co_oncoplot.png"), units = "in", res = 300, width = 14, height = 8)
coOncoplot(m1 = merged_maf, m2 = maftools_pdac, m1Name = 'PDO_TISSUE', m2Name = 'TCGA_PAAD', genes = shared_cosmic_genes, removeNonMutated = TRUE)
dev.off()
```
