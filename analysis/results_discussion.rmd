...
## Analysis of whole Exome sequencing data {#sarekMethod}
...
```{r variantcallSummary, include=TRUE, fig.cap="Summary of filtering steps used in short "}
...
As shown in the upSet plot (Figure \@ref(fig:tcgaUpsetAll)), we found that around 26% of all mutated genes were shared between both datasets.
...
50 of them (31%) were also shared with the COSMIC genes in the TCGA dataset. Importantly, the co-oncoplots in Figure \@ref(fig:tcgaCoOncoplot) show that the known driver genes such as KRAS (67% TCGA vs 92% PDO/TISSUE), TP53 (62% TCGA and PDO/TISSUE), CDKN2A (21% TCGA and 26% PDO/TISSUE) and SMAD4 (21% TCGA and 20% PDO/TISSUE) driver gene mutations were recapitulated in our PDAC/TISSUE samples. On the other hand, our PDO/TISSUE samples have a greater proportion of ARID1A (5% TCGA and 32% PDO/TISSUE) and ATM (4% TCGA and 20% PDO/TISSUE) mutations. 
...
Across the PDAC samples, we observed that all samples possess the KRAS mutation.
Of the 46 samples with the KRAS mutation, 24 samples (52%) exhibit G12V,  16 (35%)
G12V and 5 G12R (11%). We validated our WES results with Sanger Sequencing at the KRAS loci. As shown in Figure \@ref(fig:wesSanger)), most of the
...
patient harboring a different mutation (See `TP53_lollipop.png` for the range of mutations displayed).

Other genes that were mutated in relatively high proportion include the ARID1A (32%), ATM (20%), 
CDKN2A (26%) and SMAD4 (20%) mutations. The oncoplot shown in Figure \@ref(fig:oncoplotCosmic) shows the
mutated genes that were also present in the COSMIC database, thus emphasising known mutations with causal implications
in carcinogenesis. 


```{r oncoplotCosmic, include=TRUE}
...
```{r unsupervisedClustering, include=TRUE}
...
that are mutated in the samples (Figure \@ref(fig:oncoplotCNCosmic) ). The TMB score is indicated in the top panel (identical result to `tmb_pdac.png`).

```{r oncoplotCN, include=TRUE}
knitr::include_graphics(paste0(res_path_report, "oncoplot_CN.png"))
```

```{r oncoplotCNCosmic, include=TRUE}
